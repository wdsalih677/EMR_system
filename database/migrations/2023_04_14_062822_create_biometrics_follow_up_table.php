...
class CreateBiometricsFollowUpTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('biometrics_follow_ups', function (Blueprint $table) {
            $table->id();
            $table->bigInteger('patientFollowUp_id')->unsigned();
            $table->foreign('patientFollowUp_id')->references('id')->on('patient_follow_ups')->onUpdate('cascade')->onDelete('cascade');
    ...
    {
        Schema::dropIfExists('biometrics_follow_up');
    }
}
